...
function onAfterLogin() {
  ...
  // Update badge feedback (dari Supabase)
  updateFeedbackBadge();
  // Load titik tambahan admin dari Supabase
...
/* ═══════════════════════════════════════════════════════════
   FEEDBACK FORM — kini tersimpan ke Supabase (PostgreSQL)
═══════════════════════════════════════════════════════════ */
async function submitFeedback(e) {
  ...
  try {
    const { error } = await db.from('feedback').insert([entry]);
    ...
    console.error('Gagal mengirim feedback ke Supabase:', err);
    alert('Gagal mengirim feedback. Periksa koneksi internet Anda dan coba lagi.\n\nDetail: ' + (err.message || err));
  } finally {
    if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Kirim Masukan'; }
  }
}
...
// Update feedback badge dari Supabase (jumlah feedback yang masuk)
async function updateFeedbackBadge() {
  ...
  try {
    ...
    console.warn('Gagal memuat jumlah feedback:', err);
  }
}
...
kondisi         : document.getElementById('at-kondisi').value || '-',
deskripsi       : document.getElementById('at-deskripsi').value.trim() || '',
// Format PostGIS: SRID=4326;POINT(longitude latitude)
lokasi          : `SRID=4326;POINT(${lng} ${lat})`,
};

try {
  const { data, error } = await db.from('titik_tambahan').insert([row]).select();
  if (error) throw error;

  // Bentuk ulang sebagai GeoJSON Feature untuk langsung ditampilkan di peta
  const newFeature = {
    type: 'Feature',
    geometry: { type: 'Point', coordinates: [lng, lat] },
    properties: {
      nama: row.nama, kategori: row.kategori, status: row.status,
      jumlah_lantai: row.jumlah_lantai, jam_operasional: row.jam_operasional,
      kondisi: row.kondisi, deskripsi: row.deskripsi,
      _custom: true, _id: data && data[0] ? data[0].id : null,
    }
  };

  injectCustomTitikKeMap(newFeature);
  closeTambahTitik();
  showToastAdmin('✓ Titik "' + nama + '" berhasil disimpan ke database!');
} catch (err) {
  console.error('Gagal menyimpan titik ke Supabase:', err);
  errEl.textContent = '⚠ Gagal menyimpan ke database: ' + (err.message || err);
  errEl.style.display = 'block';
}
}
...
async function loadCustomTitikFromStorage() {
try {
  const { data, error } = await db.from('titik_tambahan_geojson').select('*');
  if (error) throw error;
  (data || []).forEach(function(row) {
    const feature = {
      ...
      properties: {
        ...
        jumlah_lantai: row.jumlah_lantai, jam_operasional: row.jam_operasional,
        kondisi: row.kondisi, deskripsi: row.deskripsi,
        _custom: true, _id: row.id,
      }
    };
    injectCustomTitikKeMap(feature);
  });
} catch (err) {
  console.error('Gagal memuat titik tambahan dari Supabase:', err);
}
}
/* ═══════════════════════════════════════════════════════════
...
container.innerHTML = '<div class="feedback-empty"><i class="fas fa-spinner fa-spin"></i><p>Memuat feedback...</p></div>';

const { data: list, error } = await db.from('feedback').select('*').order('created_at', { ascending: false });

if (error) {
  container.innerHTML = '<div class="feedback-empty"><i class="fas fa-exclamation-triangle"></i><p>Gagal memuat feedback dari database.</p></div>';
...
const waktu = f.created_at ? new Date(f.created_at).toLocaleString('id-ID') : '-';
rows += '<tr>' +
'<td>' + (i + 1) + '</td>' +
'<td>' + escHtml(f.nama || '-') + '</td>' +
'<td>' + escHtml(f.instansi || '-') + '</td>' +
'<td class="feedback-saran">' + escHtml(f.saran || '-') + '</td>' +
'<td class="feedback-waktu">' + waktu + '</td>' +
'<td><button class="btn-hapus-row" onclick="hapusFeedbackById(' + f.id + ')" title="Hapus"><i class="fas fa-trash-alt"></i></button></td>' +
'</tr>';
});
...
async function hapusFeedbackById(id) {
if (!confirm('Hapus feedback ini?')) return;
const { error } = await db.from('feedback').delete().eq('id', id);
if (error) { alert('Gagal menghapus feedback: ' + error.message); return; }
updateFeedbackBadge();
renderFeedbackList();
}
async function hapusSemuaFeedback() {
if (!confirm('Yakin ingin menghapus SEMUA feedback? Tindakan ini tidak bisa dibatalkan.')) return;
const { error } = await db.from('feedback').delete().neq('id', 0); // hapus semua row
if (error) { alert('Gagal menghapus semua feedback: ' + error.message); return; }
updateFeedbackBadge();
renderFeedbackList();
}
